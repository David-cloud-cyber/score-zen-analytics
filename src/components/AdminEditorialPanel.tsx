...
import { Bot, Check, MessageCircle, Play, X } from "lucide-react";
import { AdminCard } from "@/components/AdminShell";
import { isLocalDemo } from "@/lib/local-demo";
import { getAdminEditorialComments, getAdminEditorialQueue, moderateEditorialComment, runAdminEditorialCycle, setAdminEditorialStatus } from "@/lib/editorial.functions";
...
export function AdminEditorialPanel() {
  ...
  const getComments = useServerFn(getAdminEditorialComments);
  const moderate = useServerFn(moderateEditorialComment);
  const query = useQuery({ queryKey: ["admin", "editorial"], queryFn: () => getQueue(), enabled: !demo, staleTime: 30_000 });
  const commentsQuery = useQuery({ queryKey: ["admin", "editorial-comments"], queryFn: () => getComments(), enabled: !demo, staleTime: 30_000 });
  ...
  return (
    <div className="space-y-4">
      ...
      <AdminCard>
        <div className="flex items-center justify-between gap-3"><div className="flex items-center gap-2"><MessageCircle className="size-4 text-brand" /><div><p className="text-sm font-black">Commentaires à vérifier</p><p className="mt-1 text-xs text-muted-foreground">Les lecteurs peuvent discuter sans exposer leur adresse email.</p></div></div><span className="rounded-full bg-brand/10 px-2.5 py-1 text-xs font-black text-brand">{(commentsQuery.data ?? []).filter((comment: any) => comment.status === "pending" || comment.status === "reported").length}</span></div>
        <div className="mt-4 space-y-2">{(commentsQuery.data ?? []).filter((comment: any) => comment.status === "pending" || comment.status === "reported").slice(0, 6).map((comment: any) => <div key={comment.id} className="rounded-xl bg-surface p-3"><div className="flex items-center justify-between gap-2"><p className="text-xs font-black">{comment.authorName}</p><span className="text-[10px] font-bold text-brand">{comment.status}</span></div><p className="mt-2 line-clamp-2 text-xs text-muted-foreground">{comment.body}</p><div className="mt-3 flex gap-2"><button type="button" onClick={() => commentStatus.mutate({ commentId: comment.id, status: "approved" })} disabled={commentStatus.isPending} className="inline-flex items-center gap-1 rounded-lg bg-brand px-2.5 py-2 text-[10px] font-black text-brand-foreground"><Check className="size-3" />Approuver</button><button type="button" onClick={() => commentStatus.mutate({ commentId: comment.id, status: "hidden" })} disabled={commentStatus.isPending} className="inline-flex items-center gap-1 rounded-lg bg-alert px-2.5 py-2 text-[10px] font-black text-white"><X className="size-3" />Masquer</button></div></div>)}{!(commentsQuery.data ?? []).some((comment: any) => comment.status === "pending" || comment.status === "reported") ? <p className="rounded-xl bg-surface p-4 text-center text-xs font-bold text-muted-foreground">Aucun commentaire à vérifier.</p> : null}</div>
      </AdminCard>
      <AdminCard>
