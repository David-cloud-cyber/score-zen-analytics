  ...
  return <div className="min-h-screen bg-background text-foreground">
    ...
    <aside className={cn("fixed inset-y-0 left-0 z-50 flex w-[260px] flex-col border-r border-border/70 bg-card transition-transform lg:translate-x-0", collapsed && "lg:w-[76px]", mobileOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0")}>
      <div className="flex h-16 items-center justify-between border-b border-border/60 px-4">
        ...
        <button type="button" onClick={() => setCollapsed((value) => !value)} className="hidden size-9 place-items-center rounded-xl border border-border/70 lg:grid" aria-label="Réduire la sidebar">{collapsed ? <ChevronRight className="size-4" /> : <ChevronLeft className="size-4" />}</button>
        <button type="button" onClick={() => setMobileOpen(false)} className="grid size-9 place-items-center rounded-xl border border-border/70 lg:hidden" aria-label="Fermer"><X className="size-4" /></button>
      </div>
      ...
      <div className={cn("border-t border-border/60 p-3", collapsed && "lg:px-2")}>
        ...
        <button type="button" onClick={() => void signOut()} className="mt-2 flex w-full items-center gap-3 rounded-xl px-3 py-2 text-xs font-bold text-muted-foreground hover:bg-surface hover:text-foreground"><LogOut className="size-4" /><span className={cn(collapsed && "lg:hidden")}>Déconnexion</span></button>
      </div>
    </aside>
    <div className={cn("min-h-screen transition-[padding] lg:pl-[260px]", collapsed && "lg:pl-[76px]")}>
      <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-border/60 bg-background/90 px-4 backdrop-blur-xl lg:px-8"><div className="flex items-center gap-3"><button type="button" onClick={() => setMobileOpen(true)} className="grid size-9 place-items-center rounded-xl border border-border/70 lg:hidden" aria-label="Ouvrir le menu"><Menu className="size-4" /></button><div><p className="text-[10px] font-black uppercase tracking-widest text-brand">Administration</p><h1 className="text-sm font-black">Pilotage LiveFoot</h1></div></div><span className="hidden items-center gap-1.5 rounded-full bg-brand/10 px-3 py-1.5 text-[10px] font-black text-brand sm:inline-flex"><Gauge className="size-3.5" /> Système protégé</span></header>
