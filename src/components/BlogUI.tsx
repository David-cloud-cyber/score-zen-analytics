...
const EMPTY_EDITORIAL_COMMENTS: EditorialComment[] = [];
...
function ArticleActions({ articleId, title }: { articleId: string; title: string }) {
  ...
  return <div className="flex flex-wrap items-center gap-2" aria-label="Actions de l’article">
    ...
    <button type="button" onClick={() => { void navigator.clipboard.writeText(window.location.href).then(() => setMessage("Lien copié")).catch(() => setMessage("Copie indisponible")); }} className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-border bg-surface px-3 py-2 text-xs font-black text-foreground hover:border-brand/50"><LinkIcon className="size-4" aria-hidden />Copier le lien</button>
...
function EditorialComments({ articleId }: { articleId: string }) {
  ...
  const mutation = useMutation({ mutationFn: () => post({ data: { articleId, parentId: replyTo?.id ?? null, content } }), onSuccess: async () => { setContent(""); setReplyTo(null); await queryClient.invalidateQueries({ queryKey: ["editorial-comments", articleId] }); } });
  const comments = query.data?.comments ?? EMPTY_EDITORIAL_COMMENTS;
  ...
  useEffect(() => {
    const update = () => {
      ...
      frameRef.current = window.requestAnimationFrame(() => {
        ...
        if (rounded !== lastReadingRef.current) {
          lastReadingRef.current = rounded;
          setReading(percent);
        }
        if (session && !demo && saveTimer.current === null) saveTimer.current = window.setTimeout(() => { saveTimer.current = null; void saveProgress({ data: { articleId: article.id, position: Math.round(window.scrollY), percent } }).catch(() => {}); }, 1200);
      });
    };
    update(); window.addEventListener("scroll", update, { passive: true }); return () => { window.removeEventListener("scroll", update); if (frameRef.current !== null) window.cancelAnimationFrame(frameRef.current); if (saveTimer.current !== null) window.clearTimeout(saveTimer.current); };
