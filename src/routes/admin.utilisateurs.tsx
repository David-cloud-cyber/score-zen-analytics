...
import { getAdminUsers, restoreAdminUser, suspendAdminUser } from "@/lib/admin.functions";
...
function AdminUsersPage() {
  ...
  const query = useQuery({
    ...
    queryFn: () => getUsers({ data: { page: 1, pageSize: 50, search } }),
    enabled: !demo,
  });
        ...
        <AdminCard className="overflow-x-auto p-0">
          <table className="w-full min-w-[760px] text-left text-xs">
            <thead className="border-b border-border/60 bg-surface">
              <tr>
                {["Utilisateur", "Plan", "Crédits", "Rôle", "État", "Action"].map((label) => (
            ...
            <tbody className="divide-y divide-border/60">
              {(data?.users ?? []).map((user) => (
                <tr key={user.id}>
                  ...
                  <td className="p-3 font-bold">{user.plan}</td>
                  <td className="p-3 font-black text-brand">{user.credits}</td>
                  <td className="p-3">
                    {user.roles.map((role) => (
                      <span
                        key={role}
                    ...
                    {selected === user.id ? (
                      <div className="flex min-w-[260px] items-center gap-2">
                        <input
                          ...
                          className="h-8 flex-1 rounded-lg border border-border bg-background px-2 text-[10px]"
                        />
                        <button
                          type="button"
                          onClick={() =>
                          ...
                          className="rounded-lg bg-foreground px-2 py-2 text-[10px] font-black text-background"
                        >
                          Confirmer
                        </button>
                      </div>
