...
export const Route = createFileRoute("/blog/categorie/$categorie")({
  loader: async ({ params }) => {
    try {
      return await getBlogIndex({ data: { page: 1, category: params.categorie } });
    } catch {
  ...
  head: ({ params, loaderData }) => {
    const label =
      BLOG_CATEGORY_LABELS[params.categorie as keyof typeof BLOG_CATEGORY_LABELS] ?? "Football";
    const articleCount = loaderData?.articles?.length ?? 0;
    ...
    <AppShell>
      <section className="mx-auto max-w-5xl space-y-8 px-4 pb-16 pt-8 lg:px-0">
        <header>
          ...
          <h1 className="mt-2 text-3xl font-black tracking-tight">{label}</h1>
          <p className="mt-3 max-w-2xl text-sm leading-relaxed text-muted-foreground">
            Les articles vérifiés de la rédaction LiveFoot dans cette catégorie.
          </p>
        </header>
