...
function AuthPage() {
  ...
  async function handleEmail(e: React.FormEvent) {
    ...
    try {
      ...
      toast.error(
        err instanceof Error ? err.message : "Une erreur est survenue lors de l'authentification.",
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="grid min-h-dvh place-items-center bg-background px-4 py-10">
      <div className="w-full max-w-md">
        ...
        <div className="animate-rise rounded-xl border border-border/70 bg-card p-6 shadow-none">
          ...
          <div className="my-4 flex items-center gap-3 text-[10px] font-bold uppercase tracking-widest text-muted-foreground">
            <div className="h-px flex-1 bg-border" /> ou email{" "}
...
function Field({
  ...
  return (
    <label className="block">
      <div className="mb-1 px-1 text-[10px] font-black uppercase tracking-widest text-muted-foreground">
        {label}
      </div>
...
function GoogleIcon() {
  return (
    <svg className="size-4" viewBox="0 0 24 24" aria-hidden>
      ...
      <path
        fill="#34A853"
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.56-2.76c-.98.66-2.24 1.06-3.72 1.06-2.87 0-5.29-1.94-6.16-4.53H2.18v2.84A11 11 0 0 0 12 23z"
      />
      <path
        fill="#FBBC05"
      ...
      <path
        fill="#EA4335"
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1a11 11 0 0 0-9.82 6.05l3.66 2.84C6.71 7.32 9.13 5.38 12 5.38z"
      />
    </svg>
  );
}
